...
var TopicSubscriber = function (solaceModule) {
    ...
    subscriber.connect = () => new Promise((resolve) => {
        try {
        ...
        subscriber.session.on(solace.SessionEventCode.CONNECT_FAILED_ERROR, function (sessionEvent) {
            console.log("Connection failed to the message router: " +
    ...
    subscriber.subscribe = (topicName) => {
        if (subscriber.session !== null) {
            if (subscribedTo.includes(topicName)) {
                ...
                try {
                    subscriber.session.subscribe(solace.SolclientFactory.createTopicDestination(topicName), true, // generate confirmation when subscription is added successfully
    ...
    subscriber.unsubscribe = (topicName) => {
        if (subscriber.session !== null) {
            if (subscribedTo.includes(topicName)) {
                ...
                try {
                    subscriber.session.unsubscribe(solace.SolclientFactory.createTopicDestination(topicName), true, // generate confirmation when subscription is removed successfully
                    topicName, // use topic name as correlation key
                    10000 // 10 seconds timeout for this operation
                    );
                }
